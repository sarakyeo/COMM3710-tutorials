...
Let's say we want to create a new variable called `citygm` that categorizes the cars based on city gas mileage, `cty`. We will have cars with 15 miles per gallon or lower be categorized as `low` and cars with over 15 miles per gallon be categorized as `high`.
...
## Run a fequency distribution to check the new variable
freq(mpg$citygm)
```
